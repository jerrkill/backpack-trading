...
const init = async (client) => {
    try {
        ...
        console.log(getNowFormatDate(), "Waiting 1 seconds...");
        await delay(300);
...
const sellfun = async (client) => {
    let GetOpenOrders = await client.GetOpenOrders({ symbol: SYMBOL_PAIR });
    if (GetOpenOrders.length > 0) {
        let CancelOpenOrders = await client.CancelOpenOrders({ symbol: SYMBOL_PAIR });
    ...
    let { lastPrice: lastPriceask } = await client.Ticker({ symbol: SYMBOL_PAIR });
...
const buyfun = async (client) => {
    ...
    let { lastPrice } = await client.Ticker({ symbol: SYMBOL_PAIR });
